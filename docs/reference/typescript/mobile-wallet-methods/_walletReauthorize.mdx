...
Non-privileged method. Like `authorize`, this method attempts to put the current session in an authorized state, but
provides a previously issued `auth_token`. The session is authorized with the privileges associated with the specified `auth_token`.
